...
The first thing a user of ManageIQ does it to log in. Therefore if there are any issues with authentication configuration
...
This blog post attempts to provide some guidelines for troubleshooting Containerized ManageIQ authentication issue.
...
For instructions on creatng the ManageIQ CR see: [ManageIQ Operator README](https://github.com/ManageIQ/manageiq-pods/blob/master/manageiq-operator/README.md#configuring-openid-connect-authentication)
...
Therefore the httpd pod's log file may contain any errors related to mod_auth_openidc configuration.
...
The follwing snipet from the [Using the API with OpenID-Connect Authentication](https://www.manageiq.org/blog/2020/09/oidc-auth-api-usage/) blog post show the commands that will
...
If an issue is encountered where a specific user is unable to log in it may be valuable to confirm if this introspection can be performed for the failing user.
...
the user claims returned from Apache's mod-auth-openidc module will be written to the UI pods log.
...
The following example shows how to check for the users request headers from the ui pod's logs.

  ```bash
  $ oc login -u admin -p <pw> https://<my-container-host>/
  $ oc logs -f 1-ui-abcdef1234-12345
  _content omitted_
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug) user_details_from_headers(username=jvlcek)"}
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug) External-Auth remote user request.headers:"}
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug)   X-REMOTE-USER            = \"jvlcek\""}
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug)   X-REMOTE-USER-FIRSTNAME  = \"Joe\""}
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug)   X-REMOTE-USER-LASTNAME   = \"VLcek\""}
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug)   X-REMOTE-USER-FULLNAME   = \"Joe VLcek\""}
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug)   X-REMOTE-USER-EMAIL      = \"jvlcek@jvlcek.redhat.com\""}
  ... "message":"MIQ(Authenticator::Httpd#log_auth_debug)   X-REMOTE-USER-DOMAIN     = \"jvlcek.redhat.com\""}
...
If an issue is encountered it is important to capturing the logs when the issue is reproduced and not just
...
the issue is being recreated.
  ...
  While the above commands are running in seperate windows reproduce the authenctication issue.
  Then save the output of the commands and include them in the issue report.

  It would also be valuable Capture the Apache OpenID-Connect configuration file in the issue report.
...
When a user being authenticate belongs to a large number of groups, on the order of hundreds,
